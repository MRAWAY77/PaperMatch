...
"""
Simple Academic Paper Summary Extractor

This script extracts ALL academic paper summaries and their metadata 
...
def extract_academic_summaries(output_file="academic_paper_summaries_simple.xlsx"):
    """
    Extract all academic paper summaries and metadata and save to Excel.
    
    Args:
        output_file (str): Path to save the output Excel file.
    """
    print("Simple Academic Paper Summary Extractor")
    print("=======================================")
    ...
    # Step 5: Filter for academic papers
    academic_summaries = {title: data for title, data in all_summaries.items() 
                         if data.get('type', '').lower() == 'academic'}
    
    print(f"Found {len(academic_summaries)} academic paper summaries")
    
    # Step 6: Prepare data for Excel
    academic_data = []
    
    for title, data in tqdm(academic_summaries.items(), desc="Preparing data"):
        ...
        academic_data.append(paper_data)
    
    # Create DataFrame
    if not academic_data:
        print("No academic paper summaries found.")
        return
    
    df = pd.DataFrame(academic_data)
    
    # Step 7: Save to Excel
    print(f"\nSaving {len(df)} academic paper summaries to {output_file}...")
    try:
...
if __name__ == "__main__":
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Extract academic paper summaries and metadata to Excel")
    parser.add_argument('--output', '-o', type=str, default="academic_paper_summaries_simple.xlsx",
    ...
    extract_academic_summaries(args.output)
