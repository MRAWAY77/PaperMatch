...
def generate_pdf_report(results, query, topic, pdf_path):
    ...
    for item in results:
        ...
        # Add long summary as plain paragraph
...
# Function to extract the integer after 'query'
def extract_query_number(path):
    match = re.search(r'query(\d+)_', path)
...
def main():
    ...
    for query_idx, info in enumerate(sorted_paths):
        ...
        output_pdf = os.path.join("gemma3_eval_logs", output_filename)
