...
from config import ACADEMIC_EMBEDDING_PATH, NEWS_EMBEDDING_PATH, TOPIC_THRESHOLD
...
def keyword_classifier(cleaned_query: str) -> str:
    ...
    query_embedding = embedding_model.encode(cleaned_query_lower, convert_to_tensor=True)
...
def find_top_matches(query_embedding: torch.Tensor, embeddings: List[torch.Tensor], names: List[str], clusters: List[str], top_k: int = 3) -> List[Tuple[str, str]]:
    ...
    for emb, name, cluster in embeddings_zipped:
        ...
        cos_sim_score = util.cos_sim(query_embedding, emb.unsqueeze(0))[0].item() 
...
def process_query(query: str, result_callback=None):
    ...
    if classified_topic == "Unknown":
        ...
        academic_path = ACADEMIC_EMBEDDING_PATH.format(topic=classified_topic)
        news_path = NEWS_EMBEDDING_PATH.format(topic=classified_topic)
